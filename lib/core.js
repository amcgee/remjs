...
var PluginBuilder = require('./plugins');
...
var REM = function(options) {
	...
	_.forEach( _.extend({},options.resources), function( resource_options, resource_name ) {
		resource_options = _.extend( _.extend({ engine: options.engine }, resource_options), { name: resource_name } );
		resources[resource_name] = new REMResource( resource_options );
	} );

	var router = express.Router();
	var plugins = PluginBuilder(resources, options);
