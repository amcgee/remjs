...
Authenticator.prototype.meRouter = function() {
	...
	meRouter.get( "/", function(req, res) {
		return res.status( 200 ).send( req.authentication.identity );
	})
	...
	meRouter.post( "/_password", function(req, res) {
		...
		this.loginHelper.update( req.authentication.identity[this.loginHelper.authority.login_property], req.body.old_password, req.body.new_password, {}, function(err) {
			if (err) {
	...
	meRouter.delete( "/_password", function(req,res) {
		if ( !req.body || !req.body.old_password )
			return res.status( 400 ).send( "Invalid password reset request." );
		this.loginHelper.reset( req.authentication.identity[this.loginHelper.authority.login_property], req.body.old_password, function(err,pwd) {
			if (err) {
				return res.status(500).send("Failed to update password.");
			}
	...
	this.jwt.parse( req, function( err, candidate ) {
		if ( err || !candidate )
		{
			...
			this.loginHelper.lookup( candidate, function(err, identity) {
				if ( err ) {
					return res.status( 401 ).set('WWW-Authenticate','Bearer').send( "Authentication failed." );
				}
				req.authentication = {
					identity: identity
				}
