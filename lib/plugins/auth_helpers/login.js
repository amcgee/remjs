...
var LoginHelper = function(options, resources) {
    ...
    // }
        ...
        throw new Error(
        "Unknown authentication authority type '" + this.authority.type + "'");
    }
...
LoginHelper.prototype.update =
    ...
    .then( function(result) {
        ...
        return this.authority.resource.db.update(
            {'_id': identity._id}, { $set: updated_login }, {} )
        .then( function(numReplaced) {
            if ( numReplaced === 0 ) {
                throw new Error(
                "Expected to replace exactly 1 document, %d documents replaced.",
                numReplaced );
            }
            return numReplaced;
        })
